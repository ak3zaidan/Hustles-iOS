...
struct MusicProgressSlider<T: BinaryFloatingPoint>: View {
    ...
    var body: some View {
        GeometryReader { bounds in
            ZStack {
                RoundedRectangle(cornerRadius: 5).foregroundStyle(.gray).opacity(0.001)
                VStack {
                    ZStack(alignment: .center) {
                        ...
                        Capsule()
                            ...
                            .animation(.easeInOut, value: localRealProgress)
                    }
                    
                    HStack {
                        Text(formatTime(seconds: Double(value)))
                        Spacer(minLength: 0)
                        Text(formatTime(seconds: Double(inRange.upperBound)))
                    }
