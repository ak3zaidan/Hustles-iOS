...
struct groupsSend: Identifiable, Hashable {
    let id: String
    let title: String
}

struct userSend: Identifiable, Hashable {
    let id: String
    let username: String
}

struct SendToPeople: View {
    ...
    func finalSend(){
        if let link = preSavedUrlString, !link.isEmpty {
            sendMain(link: link)
        } else {
            ImageUploader.uploadImage(image: image, location: "shared", compression: 0.15) { link, _ in
               sendMain(link: link)
            }
        }
    }
    func sendMain(link: String) {
        if !link.isEmpty {
            ...
            if addToStory {
                if let coords = viewModel.postStoryLoc {
                    addStory(location: myLoc(country: "", state: "", city: "", lat: coords.0, long: coords.1), imageURL: link)
                    viewModel.postStoryLoc = nil
                } else if let loc = globeViewModel.currentLocation {
                    addStory(location: loc, imageURL: link)
                }
            }
            sendOtherContent(imageURL: link)
            if isMain {
    ...
    func addStory(location: myLoc, imageURL: String) {
        if let user = auth.currentUser {
            let size = widthOrHeight(width: false)
            let pos = (size - position) / size
            
            let postID = "\(UUID())"
            globeViewModel.uploadStoryImage(caption: caption, captionPos: pos, link: nil, image: image, id: postID, uid: user.id ?? "", username: user.username, userphoto: user.profileImageUrl, infinite: infinite == true ? true : nil, optionalLoc: location)
            
            if let x = profile.users.firstIndex(where: { $0.user.id == user.id }) {
                let new = Story(id: postID, uid: user.id ?? "", username: user.username, profilephoto: user.profileImageUrl, long: location.long, lat: location.lat, text: caption.isEmpty ? nil : caption, textPos: pos, imageURL: imageURL, timestamp: Timestamp(), link: nil, geoHash: "")
    ...
    func sendOtherContent(imageURL: String){
        for i in 0..<allG.count {
            groupViewModel.uploadStory(caption: caption, image: imageURL, groupID: allG[i].id, username: auth.currentUser?.username ?? "", profileP: auth.currentUser?.profileImageUrl)
        }
        if !allGCs.isEmpty {
            let id = String((auth.currentUser?.id ?? "").prefix(6)) + String("\(UUID())".prefix(10))
            
            let new = GroupMessage(id: id, seen: nil, text: caption.isEmpty ? nil : caption, imageUrl: imageURL, audioURL: nil, videoURL: nil, file: nil, replyFrom: nil, replyText: nil, replyImage: nil, replyFile: nil, replyAudio: nil, replyVideo: nil, countSmile: nil, countCry: nil, countThumb: nil, countBless: nil, countHeart: nil, countQuestion: nil, timestamp: Timestamp())
            
            for i in 0..<allGCs.count {
                if let index = groupChats.chats.firstIndex(where: { $0.id == allGCs[i].id }) {
                    GroupChatService().sendMessage(docID: allGCs[i].id, text: caption, imageUrl: imageURL, messageID: id, replyFrom: nil, replyText: nil, replyImage: nil, replyVideo: nil, replyAudio: nil, replyFile: nil, videoURL: nil, audioURL: nil, fileURL: nil, exception: false, lat: nil, long: nil, name: nil, choice1: nil, choice2: nil, choice3: nil, choice4: nil, pinmap: nil)
        ...
        for i in 0..<allU.count {
            if allU[i].id != "lQTwtFUrOMXem7UXesJbDMLbV902" {
                ...
                if let index = viewModel.chats.firstIndex(where: { $0.user.id == allU[i].id }) {
                    let new = Message(id: id, uid_one_did_recieve: (viewModel.chats[index].convo.uid_one == auth.currentUser?.id ?? "") ? false : true, seen_by_reciever: false, text: caption.isEmpty ? nil : caption, imageUrl: imageURL, timestamp: Timestamp(), sentAImage: nil, replyFrom: nil, replyText: nil, replyImage: nil, replyELO: nil, replyFile: nil)
                    
                    viewModel.sendStory(i: index, myMessArr: auth.currentUser?.myMessages ?? [], otherUserUid: allU[i].id, caption: caption, imageUrl: imageURL, videoUrl: nil, messageID: id, audioStr: nil, lat: nil, long: nil, name: nil, pinmap: nil)
                    
                    viewModel.chats[index].lastM = new
                    
                    viewModel.chats[index].messages?.insert(new, at: 0)
                } else {
                    viewModel.sendStorySec(otherUserUid: allU[i].id, caption: caption, imageUrl: imageURL, videoUrl: nil, messageID: id, audioStr: nil, lat: nil, long: nil, name: nil, pinmap: nil)
                }
            }
        }
    }
