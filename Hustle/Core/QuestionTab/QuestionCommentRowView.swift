...
struct QuestionCommentRowView: View {
    ...
    var body: some View {
                        ...
                        if let elements = viewModel.commentReplies.first(where: { $0.0 == comment.id ?? "NA" })?.1, !elements.isEmpty && showReplies {
                            LazyVStack(alignment: .leading){
                                ForEach(elements) { comment in
                                    QuestionCommentRowViewReply(comment: comment, parentID: self.comment.id ?? "", canShowProfile: canShowProfile).padding(.top, 7)
                                }
        ...
        .alert("Are you sure you want to delete this comment", isPresented: $showDelete) {
            Button("Confirm", role: .destructive) {
                viewModel.deleteComment(commentID: comment.id, hasReps: (comment.replies ?? 0) > 0, questionID: qID, answerID: aID, isAnswer: !aID.isEmpty)
            }
...
struct QuestionCommentRowViewReply: View {
    @EnvironmentObject var viewModel: QuestionCommentModel
    @EnvironmentObject var auth: AuthViewModel
    let comment: Comment
    let parentID: String
