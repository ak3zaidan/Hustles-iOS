...
struct BasicCoinSendView: View {
    ...
    var body: some View {
        Button {
            ...
            ZStack {
                Rectangle().foregroundStyle(.gray).opacity(0.001).frame(width: 30, height: 20)
                Image(systemName: "square.and.arrow.up")
                    .foregroundStyle(colorButton).font(.system(size: 25)).offset(y: -2)
            }
        }
        .sheet(isPresented: $showForward, content: {
            SendProfileView(sendLink: $sendString)
                .presentationDragIndicator(.visible).presentationDetents([.fraction(0.65), .large])
        })
    }
}
...
struct AllStockView: View {
    ...
    @State var showAILines: Bool = false
    ...
    var body: some View {
        VStack(spacing: 5){
            ZStack(alignment: .bottom){
                HStack(spacing: 4){
                    if let idx = appModel.currentCoin {
                        let profit = appModel.coins[idx].price_change_day >= 0
                        ...
                        Button {
                            ...
                            Image(systemName: "chevron.backward")
                                .foregroundStyle(profit ? .green : .red).font(.system(size: 28))
                        }.padding(.trailing, 8)
                        
                        BasicCoinSendView(coin: "$\(symbol.uppercased())", colorButton: profit ? .green : .red)
                    } else {
                        Button {
                            viewTop = false
                            presentationMode.wrappedValue.dismiss()
                            ...
                            Image(systemName: "chevron.backward")
                                .foregroundStyle(.green).font(.system(size: 28))
                        }.padding(.trailing, 8)
                        
                        BasicCoinSendView(coin: "$\(symbol.uppercased())", colorButton: .green)
                    }
                    Spacer()
                    ZStack {
                        Capsule().foregroundStyle(Color(UIColor.lightGray))
                        HStack {
                            if showAILines {
                                Spacer()
                            }
                            Button(action: {
                                UIImpactFeedbackGenerator(style: .light).impactOccurred()
                                if !showAILines && !appModel.isGenerating {
                                    if !appModel.AIResponsePoints.contains(where: { $0.0 == selected ?? symbol }){
                                        Task {
                                            await appModel.AskQuestion(asset: selected ?? symbol, retry: true)
                                        }
                                    }
                                }
                                ...
                                ZStack {
                                    Capsule().foregroundStyle(showAILines ? .green : .red)
                                    Text("AI").font(.subheadline).bold().foregroundStyle(.white)
                    ...
                    if appModel.currentCoin != nil {
                        Button {
                            if appModel.savedStocks.contains(where: { $0.1 == selected ?? symbol }) {
                                UIImpactFeedbackGenerator(style: .medium).impactOccurred()
                                appModel.removeStock(name: name, symbol: selected ?? symbol)
                                if let x = appModel.savedStocks.firstIndex(where: { $0.1 == selected ?? symbol }) {
                                    appModel.savedStocks.remove(at: x)
                                }
                                
                                popRoot.alertImage = "bookmark.slash.fill"
                                popRoot.alertReason = "\((selected ?? symbol).uppercased()) unsaved from list"
                                withAnimation {
                                    popRoot.showAlert = true
                                }
                            } else {
                                UIImpactFeedbackGenerator(style: .medium).impactOccurred()
                                appModel.saveStock(name: name, symbol: selected ?? symbol)
                                appModel.savedStocks.append((name, selected ?? symbol))
                                
                                popRoot.alertImage = "bookmark.fill"
                                popRoot.alertReason = "\((selected ?? symbol).uppercased()) saved to list"
                                withAnimation {
                                    popRoot.showAlert = true
                                }
                            }
            ...
            .padding(.top, isSheet ? 10 : 60)
            .overlay(alignment: .bottom) {
                Divider().opacity(showHeader ? 1.0 : 0.0).offset(y: 4)
            }
        ...
        .onAppear {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
                ...
                if let final = selected {
                    if !appModel.technical.contains(where: { $0.0 == final }){
                        appModel.getTechnical(symbol: final)
                    }
                } else {
            ...
            viewTop = true
            if let final = selected {
                appModel.getIndex(symbol: final, name: "")
            } else {
        ...
        .onChange(of: popRoot.tap) { _, _ in
            if (popRoot.tap == 3 || popRoot.tap == 1) && viewTop {
                presentationMode.wrappedValue.dismiss()
                popRoot.tap = 0
            }
        }
        .onChange(of: PriceChanged) { _, _ in
            changeAllPrices()
        }
    }
    ...
    func stats(index: Int)->some View{
        VStack(spacing: 10){
            AIStocks(asset: selected ?? symbol)
            ...
            if let element = appModel.technical.first(where: { $0.0 == selected ?? symbol }) {
...
struct LineGraph: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .onChange(of: barTranslation) { _, newValue in
            ...
            .overlay (
                ZStack {
                    ...
                    if let indicators = appModel.AIResponsePoints.first(where: { $0.0 == symbol }), showLines {
                        let closestOne = data.enumerated().min(by: { abs($0.element - indicators.1) < abs($1.element - indicators.1) })?.offset
                        let closestTwo = data.enumerated().min(by: { abs($0.element - indicators.2) < abs($1.element - indicators.2) })?.offset
                        if let one = closestOne, let two = closestTwo {
                            HStack(spacing: 5){
                                Spacer()
                                Text("Sell").foregroundStyle(.green).font(.subheadline)
                                Rectangle().frame(height: 1.5).foregroundStyle(.gray)
                            }.padding(.leading, 5).position(y: points[two].y).offset(x: widthOrHeight(width: true) * 0.5)
                            HStack(spacing: 5){
                                Spacer()
                                Text("Buy").foregroundStyle(.red).font(.subheadline)
                                Rectangle().frame(height: 1.5).foregroundStyle(.gray)
                            }.padding(.leading, 5).position(y: points[one].y).offset(x: widthOrHeight(width: true) * 0.5)
                        }
                    }
