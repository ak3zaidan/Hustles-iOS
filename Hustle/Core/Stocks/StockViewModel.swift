...
class StockViewModel: ObservableObject {
    ...
    @Published var coinsSec = [CryptoModel]()
    ...
    @Published var fetchingNow = [String]()
...
//    ]
    ...
    @Published var AIResponsePoints: [(String, Double, Double)] = []
...
//        }
    ...
    func processString(_ input: String) -> (min: Double, max: Double)? {
        let components = input.components(separatedBy: CharacterSet.whitespaces)
        let cleanedComponents = components.map { component in
            var cleanedComponent = component.trimmingCharacters(in: .punctuationCharacters)
            cleanedComponent = cleanedComponent.trimmingCharacters(in: .symbols)
            return cleanedComponent
        }
        let doubles = cleanedComponents.compactMap { Double($0) }

        if let minValue = doubles.min(), let maxValue = doubles.max() {
        ...
        if let x = self.coins.firstIndex(where: { $0.symbol == asset }) {
            ...
            service.sendStreamMessage(messages: messages).responseStreamString { [weak self] stream in
                ...
                case .stream(let response):
                    ...
                    case .failure(_):
                        DispatchQueue.main.async {
                            ...
                            if let text = self.AIResponse.first?.1, let points = self.processString(text) {
                                if let index = self.AIResponsePoints.firstIndex(where: { $0.0 == asset }) {
                                    self.AIResponsePoints[index].1 = min(points.0, points.1)
                                    self.AIResponsePoints[index].2 = max(points.0, points.1)
                                } else {
                                    self.AIResponsePoints.insert((asset, min(points.0, points.1), max(points.0, points.1)), at: 0)
                                }
                            }
                        }
                    }
                case .complete(_):
                    DispatchQueue.main.async {
                        self.isGenerating = false
                        if let points = self.processString(self.AIResponse[0].1) {
                            if let index = self.AIResponsePoints.firstIndex(where: { $0.0 == asset }) {
                                self.AIResponsePoints[index].1 = min(points.0, points.1)
                                self.AIResponsePoints[index].2 = max(points.0, points.1)
                            } else {
    ...
    func getIndexSec(symbol: String){
        if !coinsSec.contains(where: { $0.symbol == symbol }){
            if let copy = self.coins.first(where: { $0.symbol == symbol }) {
                DispatchQueue.main.async {
                    self.coinsSec.insert(copy, at: 0)
                }
            } else if !fetchingNow.contains(symbol) {
                fetchingNow.append(symbol)
                DispatchQueue.main.asyncAfter(deadline: .now() + 5.0){
                    self.fetchingNow.removeAll(where: { $0 == symbol })
                }
                ...
                getMarketData(asset: symbol, days: 1, enough: false) { data in
                    ...
                    var new = CryptoModel(id: "\(UUID())", symbol: symbol, name: "", current_price: data.candleSticks.last?.close ?? 0.0, price_change_day: dayChange, price_change_day_dollar: dayChangeDollar, day_prices: arrayOfTuples, time_last_fetched: Date(), isCrypto: false, enoughPreMarketData: enoughData)
                    ...
                    DispatchQueue.main.async {
                        self.coinsSec.insert(new, at: 0)
                    }
                }
            }
        } else if let copy = self.coins.first(where: { $0.symbol == symbol }), let already = self.coinsSec.firstIndex(where: { $0.symbol == symbol }) {
            if coinsSec[already].day_prices.count < copy.day_prices.count {
                DispatchQueue.main.async {
                    self.coinsSec[already] = copy
                }
            }
        }
    }
    ...
    func startStocks(){
        if !fetchingData {
            ...
            let symbolsInArray2 = Set(self.coins.map { $0.symbol.lowercased() })
            overall = overall.filter { !symbolsInArray2.contains($0.1.lowercased()) }
    ...
    func getMarketData(asset: String, days: Int, enough: Bool, completion: @escaping (MarketDataResponse) -> Void){
        APICaller.shared().marketData(for: asset.uppercased(), numberOfDays: days, enough: enough) { result in
            switch result {
            case .success(let data):
                completion(data)
            case .failure:
